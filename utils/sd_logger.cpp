...
void sd_logger_service(SystemState &state, uint32_t now_us, uint32_t now_ms) {
  ...
  // The SD logger reconstructs the warning mask locally instead of calling the
  // telemetry module so logging remains decoupled from Serial formatting.
  uint32_t warn_mask = 0U;

  if (!state.health.bmp_ok) warn_mask |= (1UL << 0);
  if (!state.health.bmi_accel_ok) warn_mask |= (1UL << 1);
  if (!state.health.bmi_gyro_ok) warn_mask |= (1UL << 2);
  if (!state.health.lis_ok) warn_mask |= (1UL << 3);
  if (!state.baro.valid) warn_mask |= (1UL << 4);
  if (!state.imu.valid) warn_mask |= (1UL << 5);
  if (!state.attitude.valid) warn_mask |= (1UL << 8);
  if (!state.est.valid) warn_mask |= (1UL << 9);
  if ((!state.sdlog.card_ok) || state.sdlog.runtime_failed) warn_mask |= (1UL << WARN_SD_FAULT_BIT);
