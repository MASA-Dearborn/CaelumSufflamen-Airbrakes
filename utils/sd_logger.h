...
OWNERSHIP
  sd_logger.cpp owns:
    ...
    - logger-local gravity reference,
    - logger-local altitude Kalman state.

DESIGN REASON
  The SD logger is intentionally separated from telemetry. Telemetry is a live
  Serial observation channel. The SD logger is a persistent flight/test record
  channel with its own failure behavior and flush policy.
