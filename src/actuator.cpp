...
IMPLEMENTATION NOTE
  This implementation maps configured pulse-equivalent values into 
  Servo/PWMServo degrees. g_last_us remains the telemetry representation
  of the equivalent pulse command.
===============================================================================
*/
...
static int us_to_degrees(int us) {
  ...
  // The Arduino Servo API is angle-based on many targets, so the firmware maps
  // its engineering pulse-equivalent command into the backend's degree scale.
...
void actuator_write_command01(float command01) 
{
  ...
  // g_last_us records the pulse-equivalent engineering command, not the Servo
  // library's internal degree representation, so telemetry reflects the control
  // system's intended actuator quantity.
