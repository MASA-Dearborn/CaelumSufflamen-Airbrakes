...
bool estimation_update(SystemState &state, uint32_t now_ms) {
  ...
  if (state.imu.updated) {
    ...
    // updated is a one-cycle consumption flag. Clearing it prevents accidental
    // double integration if loop() iterates again before a new IMU sample
    // arrives.
  ...
  if (state.baro.updated) {
    ...
    // As with IMU updates, barometer updates are consumed exactly once.
